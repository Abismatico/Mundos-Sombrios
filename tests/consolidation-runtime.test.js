...
function progressionRuntime(active=false) {
  const d=dom(),elements=new Map();for(const id of ['ms-character-viewer','screen-vtt']){const n=d.node();n.id=id;elements.set(id,n);}
...
test('campanha possui um único construtor e Forja não redefine entrada da Mesa',()=>{
  ...
  const shell=read('js/table-room.js');assert.equal((shell.match(/function ensureCampaignWindow\(/g)||[]).length,1);
});
...
test('Mesa tem uma fonte de layout e janelas próprias em 12 larguras',()=>{
 const css=read('css/table-room.css');
 for(const file of ['css/table-studio-v2.8.6.css','css/forja-overhaul-v2.8.10.css','css/operational-control-v2.10.1.css'])assert.doesNotMatch(read(file),/ms-room-primary|ms-room-navigation|screen-vtt/,file);
 for(const width of [320,390,640,700,760,960,961,1024,1180,1181,1440,1920]){
  const values=cssValues(css,width);
  assert.equal(values['.ms-room|display'].value,'grid');assert.equal(values['.ms-room-head-actions|display'].value,'flex');assert.equal(values['.ms-room-float|position'].value,'fixed');
 }
});
