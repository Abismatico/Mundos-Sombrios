...
(function () {
  ...
  function patchEnterVTT() {
    if (window.__msProgressionEnterPatched || typeof window.enterVTT !== 'function') return;
    window.__msProgressionEnterPatched = true;
    const originalEnter = window.enterVTT;
    window.enterVTT = async function () {
    ...
    const originalLeave = window.leaveVTT;
    if (typeof originalLeave === 'function') {
      window.leaveVTT = async function () {
        const result = await originalLeave.apply(this, arguments);
        tableState = null; updateOrb();
        return result;
      };
    }
  }
  ...
  function init() {
    ...
    patchCreateTable(); patchAdminTabs(); patchEnterVTT(); patchBuilderSave();
    installRealtimeGrant();
    window.MS_PLATFORM?.on?.('vtt:entered', async detail => {
      ensureOrb(); restoreOrbPosition();
    ...
    document.addEventListener('click', event => {
      ...
      if (el && !el.closest('#ms-character-viewer')) openViewer(el.dataset.characterId, { tableId: el.dataset.tableId || ctx().table?.id || null });
    });
  }
