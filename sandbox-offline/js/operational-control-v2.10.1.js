...
(function(){
  ...
  const manager=()=>role()==='admin'||!!context().asGM||!!document.getElementById('ms-table-room')?.classList.contains('is-gm');
  ...
  let pollTimer=null,crewGeneration=0;
  ...
  function updateCrewCounter(){
    ...
    const manageBtn=document.querySelector('#ms-table-room [data-room-action="manage"]');
  ...
  async function loadCrew(silent=false){
    ...
    const generation=++crewGeneration,accountId=window.currentUser?.id;
    ...
    try{
      ...
      if(generation!==crewGeneration||String(context().table?.id)!==String(table.id)||window.currentUser?.id!==accountId||!manager())return false;
    ...
    }catch(error){if(generation!==crewGeneration||String(context().table?.id)!==String(table.id))return false;crewState.loading=false;if(!silent)renderCrewError(error);return false;}
  }
  ...
  function evoCharacterCards(){
    ...
    return `<div class="ms-op-list">${rows.map(r=>{const a=accountFor(r.characterId)||{},tracks=evoTracksFor(r.characterId),ready=tracks.filter(t=>String(t.status)==='ready').length,progress=tracks.filter(t=>String(t.status)!=='ready').length,pending=(crewState.progression?.upgradeRequests||[]).filter(x=>String(x.character_id)===String(r.characterId)&&String(x.status||'pending')==='pending').length;return `<article class="ms-op-card"><div><strong>${esc(r.characterName||r.username)}</strong><small>${Number(a.balance||0)} PEG · ${ready} pronta(s) · ${progress} em progresso${pending?` · ${pending} solicitação(ões)`:''}</small></div><div class="ms-op-actions"><button type="button" data-crew-view="${esc(r.characterId)}">VER FICHA</button><button type="button" data-evo-manage="${esc(r.characterId)}">GERENCIAR EVOLUÇÃO</button>${(context().players||[]).some(p=>String(p.sourceCharId||p.id)===String(r.characterId)&&String(p.mode).toLowerCase()==='ocultatun')?`<button type="button" data-career-char="${esc(r.characterId)}">REGISTRAR CARREIRA</button>`:''}${ready?`<button type="button" class="approve" data-evo-direct="${esc(r.characterId)}">CONCEDER EVOLUÇÃO</button>`:''}<button type="button" class="approve" data-crew-grant="${esc(r.characterId)}" data-crew-name="${esc(r.characterName)}">CONCEDER PONTOS</button><button type="button" data-crew-withdraw="${esc(r.characterId)}" data-crew-name="${esc(r.characterName)}">DEVOLVER À RESERVA</button></div></article>`}).join('')}</div>`;
  }
  function pendingLegacyRequests(){const p=crewState.progression||{};return [...(p.pendingProposals||[]).map(r=>({id:r.id,kind:'proposal',title:'Proposta de evolução',note:r.note,cost:r.requested_cost})),...(p.pendingResources||[]).map(r=>({id:r.id,kind:'resource',title:'Ajuste de '+r.resource_key,note:r.reason,cost:null}))].map(r=>`<article class="ms-op-card"><div><strong>${esc(r.title)}</strong><small>${r.cost!=null?Number(r.cost)+' PEG · ':''}${esc(r.note||'')}</small></div><div class="ms-op-actions"><button data-existing-request="${esc(r.id)}" data-kind="${r.kind}" data-approve="1">Aprovar</button><button data-existing-request="${esc(r.id)}" data-kind="${r.kind}" data-approve="0">Recusar</button></div></article>`).join('');}
  function evoRequestsPane(){
    const p=crewState.progression||{},items=[],existing=pendingLegacyRequests();
    ...
    if(!items.length)return existing||'<div class="ms-op-empty">Nenhuma solicitação de evolução pendente.</div>';
    return `<div class="ms-op-list">${items.map(x=>`<article class="ms-op-card"><div><strong>${esc(x.title)}</strong><small>${esc(x.detail)}${x.note?`<br>${esc(x.note)}`:''}</small></div><div class="ms-op-actions"><button type="button" data-evo-view-history="${esc(x.char)}">VER HISTÓRICO</button><button type="button" class="approve" data-evo-resolve="${esc(x.id)}" data-evo-kind="${esc(x.kind)}" data-evo-approved="1">ACEITAR</button><button type="button" class="danger" data-evo-resolve="${esc(x.id)}" data-evo-kind="${esc(x.kind)}" data-evo-approved="0">RECUSAR</button></div></article>`).join('')}</div>`+existing;
  }
  ...
  function bindCrewActions(root){
    root.querySelectorAll('[data-existing-request]').forEach(b=>b.onclick=async()=>{const fn=b.dataset.kind==='proposal'?'resolveProposal':'resolveResource';await window.MS_PROGRESSION[fn](b.dataset.existingRequest,b.dataset.approve==='1');await loadCrew(true);});
    root.querySelectorAll('[data-career-char]').forEach(b=>b.onclick=async()=>{const n=Number(prompt('Sucessos de Carreira obtidos por prática real:','1')||0),reason=String(prompt('Missão ou feito que gerou os sucessos:','')||'').trim();if(n<=0||!reason)return;try{await svc().Progression.recordCareer(crewState.table.id,b.dataset.careerChar,n,reason);await loadCrew(true);}catch(e){toast(errorText(e),'error');}});
  ...
  async function grantPeg(characterId,name='Participante'){
    if(await window.MS_POINTS.open('grant',{tableId:crewState.table?.id,characterId,name}))await loadCrew(true);
  }
  ...
  async function openCrewCenter(options={}){
    ...
    const modal=ensureCrewCenter();document.body.appendChild(modal);modal.classList.remove('is-embedded');modal.classList.add('open');modal.querySelector('[role=dialog]').setAttribute('aria-modal',String(!document.body.classList.contains('ms-workspace-active')));lock(true);crewState.table=clone(table);await loadCrew(false);
  }
  async function mountEvolutionWorkspace(host){
    const tableId=context().table?.id,accountId=window.currentUser?.id;
    if(!tableId){host.innerHTML='<p>Salve a Mesa para acessar a evolução.</p>';return;}
    await window.MS_FEATURES.ensureProgression();
    if(context().table?.id!==tableId||window.currentUser?.id!==accountId)return;
    if(!manager()){
      const players=window.MS_TABLE_SHEETS.visiblePlayers(context(),window.currentUser);
      host.innerHTML=`<div class="ms-op-panel"><h2>Sua evolução</h2><p>Consulte trilhas, registre práticas e envie solicitações ao Mestre pela sua ficha.</p>${players.map(p=>`<button type="button" data-own-evolution="${esc(p.sourceCharId||p.id)}">Evolução de ${esc(p.name||'personagem')}</button>`).join('')||'<p>Vincule uma ficha à Mesa para acompanhar sua evolução.</p>'}</div>`;
      host.querySelectorAll('[data-own-evolution]').forEach(b=>b.onclick=()=>window.MS_PROGRESSION.openEvolutionForCharacter(b.dataset.ownEvolution,context().table.id));return;
    }
    const modal=ensureCrewCenter();host.replaceChildren(modal);modal.classList.add('open','is-embedded');modal.querySelector('[role=dialog]').setAttribute('aria-modal','false');crewState.tab='evolution';crewState.table=clone(context().table);await loadCrew(false);
  }
  async function refreshCounter(){
    if(!manager()||!context().table?.id)return;
    const tableId=context().table.id,accountId=window.currentUser?.id,generation=crewGeneration;
    try{const [requests,progression]=await Promise.all([svc()?.Games?.joinRequests?.(tableId),svc()?.Progression?.state?.(tableId)]);
      if(generation!==crewGeneration||context().table?.id!==tableId||window.currentUser?.id!==accountId||!manager())return;
      crewState.requests=unwrap(requests)||[];crewState.progression=unwrap(progression)||crewState.progression;updateCrewCounter();if(document.getElementById('ms-crew-center')?.classList.contains('open'))renderCrew();
    }catch(_){}
  }
  ...
  function createFabricLite(){
    ...
    class CanvasLite{
      ...
      getPointer(e={}){const r=this.lowerCanvasEl?.getBoundingClientRect?.()||{left:0,top:0,width:this.width,height:this.height},sx=(Number(e.clientX||0)-r.left)*(this.width/(r.width||this.width||1)),sy=(Number(e.clientY||0)-r.top)*(this.height/(r.height||this.height||1)),v=this.viewportTransform||[1,0,0,1,0,0],a=Number(v[0]||1),d=Number(v[3]||1);return{x:(sx-Number(v[4]||0))/a,y:(sy-Number(v[5]||0))/d};}
      setBackgroundImage(img,cb,opts={}){if(img)img.set?.(opts);this.backgroundImage=img;this.requestRenderAll();cb?.();return this;}setViewportTransform(v){if(Array.isArray(v)&&v.length>=6)this.viewportTransform=v.slice(0,6).map(Number);this.requestRenderAll();return this;}
      ...
      renderAll(){const c=this.contextContainer;if(!c)return this;c.setTransform(1,0,0,1,0,0);c.clearRect(0,0,this.width,this.height);const v=this.viewportTransform||[1,0,0,1,0,0];c.save();c.setTransform(Number(v[0]||1),Number(v[1]||0),Number(v[2]||0),Number(v[3]||1),Number(v[4]||0),Number(v[5]||0));if(this.backgroundImage)this._draw(this.backgroundImage,c);for(const o of this._objects)if(o.visible!==false)this._draw(o,c);c.restore();return this;}
  ...
  function bindRuntime(){
    ...
    window.MS_PLATFORM?.on?.('progression:state-refreshed',e=>{if(String(e?.tableId)===String(context().table?.id))refreshCounter();});
    window.MS_PLATFORM?.on?.('vtt:left',()=>{crewGeneration++;stopPolling();closeCrewCenter();crewState.table=null;crewState.roster=[];crewState.progression=null;});
  ...
  window.MS_OPERATIONAL_CONTROL=Object.freeze({version:'2.10.1',mountEvolutionWorkspace,openQuickSheets,openCrewCenter,loadCrew,refreshCounter,createFabricLite,getState:()=>clone(crewState)});
