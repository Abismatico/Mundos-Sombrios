...
(function(){
  ...
  const loaderOpen=async function(){
    ...
    if(root&&!window.msShieldNavigate&&!document.getElementById('ms-shield-loading'))root.querySelector('.master-shield-body')?.insertAdjacentHTML('afterbegin','<div id="ms-shield-loading" class="panel"><b>ABRINDO ACERVO RESTRITO…</b><p>Carregando cronologia, cartografia e compêndio somente agora.</p></div>');
  ...
  function load(src,id){return window.MS_ASSETS.script(src,{id});}
