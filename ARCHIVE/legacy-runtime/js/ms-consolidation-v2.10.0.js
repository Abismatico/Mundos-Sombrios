/* Mundos Sombrios — Release Contract V2.10.0
...
(function(){
  ...
  function selfCheck(){
    ...
    const result={version:'2.10.0',ok:missing.length===0,missing,offline:db?.offline===true,sandbox:window.MS_RUNTIME_CONFIG?.sandboxMode===true};
    window.dispatchEvent(new CustomEvent('ms:release-check',{detail:result}));
    if(!result.ok){console.error('[Mundos Sombrios V2.10.0] Contratos ausentes:',missing);window.MS_PLATFORM?.setStatus?.('release','error',new Error(missing.join(', ')));}
    else window.MS_PLATFORM?.setStatus?.('release','success');
    return result;
  }
  window.MS_RELEASE=Object.freeze({version:'2.10.0',requiredDb:[...requiredDb],features:[...features],selfCheck});
