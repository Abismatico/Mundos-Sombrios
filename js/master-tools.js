...
(function(){
  ...
  async function renderMasterTools(root, table=null){
    if(!gm()) return;
  ...
  function renderNPCs(panel){
    ...
    panel.innerHTML=`<div class="gm-npc-grid"><form class="gm-npc-form" id="gm-npc-form"><input id="npc-name" required placeholder="Nome do NPC"><input id="npc-role" placeholder="Função / ameaça"><div class="gm-npc-stats"><input id="npc-pv" type="number" min="0" placeholder="PV"><input id="npc-def" type="number" placeholder="Defesa"><input id="npc-cd" type="number" placeholder="CD"><input id="npc-mode" value="${mode()}" readonly></div><textarea id="npc-attacks" rows="2" placeholder="Ataques / ações"></textarea><textarea id="npc-resistances" rows="2" placeholder="Resistências / vulnerabilidades"></textarea><textarea id="npc-powers" rows="2" placeholder="Poderes / recursos"></textarea><textarea id="npc-behavior" rows="2" placeholder="Comportamento e tática"></textarea><textarea id="npc-objectives" rows="2" placeholder="Objetivos e interesses"></textarea><input id="npc-token" placeholder="Token vinculado / identificação"><textarea id="npc-notes" rows="3" placeholder="Notas adicionais"></textarea><button type="submit">REGISTRAR NPC</button></form><div class="gm-npc-list" id="gm-npc-list"></div></div>`;
    const list=panel.querySelector('#gm-npc-list'); if(!npcs.length){list.innerHTML='<div class="gm-empty">Nenhum NPC registrado.</div>';} else npcs.forEach(n=>{list.insertAdjacentHTML('beforeend',`<article class="gm-npc-card"><div><b>${esc(n.name)}</b><small>${esc(n.role||'NPC')} · ${esc(n.mode)}</small></div><p>PV ${esc(n.pv??'—')} · DEF ${esc(n.def??'—')} · CD ${esc(n.cd??'—')}</p><p><b>Ações:</b> ${esc(n.attacks||'—')}<br><b>Comportamento:</b> ${esc(n.behavior||'—')}<br><b>Objetivo:</b> ${esc(n.objectives||'—')}</p><div><button type="button" data-edit-npc="${esc(n.id)}">EDITAR</button><button type="button" class="danger" data-delete-npc="${esc(n.id)}">EXCLUIR</button></div></article>`) });
    panel.querySelector('#gm-npc-form').addEventListener('submit',e=>{e.preventDefault();const get=id=>panel.querySelector('#'+id)?.value?.trim()||'';const name=get('npc-name');if(!name)return;const data={id:crypto.randomUUID?crypto.randomUUID():String(Date.now()+Math.random()),name,role:get('npc-role'),pv:Number(get('npc-pv')||0),def:Number(get('npc-def')||0),cd:Number(get('npc-cd')||0),mode:mode(),attacks:get('npc-attacks'),resistances:get('npc-resistances'),powers:get('npc-powers'),behavior:get('npc-behavior'),objectives:get('npc-objectives'),tokenLink:get('npc-token'),notes:get('npc-notes'),updatedAt:Date.now()};setScoped(NPCS,[...scoped(NPCS),data]);renderNPCs(panel);});
    ...
    panel.querySelectorAll('[data-edit-npc]').forEach(b=>b.addEventListener('click',()=>{const n=scoped(NPCS).find(x=>String(x.id)===String(b.dataset.editNpc));if(!n)return;['name','role','pv','def','cd','attacks','resistances','powers','behavior','objectives','notes'].forEach(k=>{const el=panel.querySelector('#npc-'+k);if(el)el.value=n[k]??'';});const tok=panel.querySelector('#npc-token');if(tok)tok.value=n.tokenLink||'';setScoped(NPCS,scoped(NPCS).filter(x=>String(x.id)!==String(n.id)));}));
  }
  function workbench(){const s=vttState();s.gmWorkbench=s.gmWorkbench||{encounters:[],combat:{round:1,active:0,entries:[]},clues:[],world:[]};return s.gmWorkbench;}
  ...
  function renderCombat(panel){const w=workbench(),c=w.combat||(w.combat={round:1,active:0,entries:[]});panel.innerHTML=`<div class="master-card"><div class="gm-tool-toolbar"><button id="combat-prev">◀ TURNO</button><b>RODADA ${Number(c.round)||1}</b><button id="combat-next">PRÓXIMO ▶</button></div><form id="combat-add" class="ms-table-meta-grid"><input id="combat-name" required placeholder="Combatente"><input id="combat-init" type="number" placeholder="Iniciativa"><input id="combat-hp" type="number" placeholder="PV"><input id="combat-cond" placeholder="Condições"><button class="ms-span-2">ADICIONAR À ORDEM</button></form><div class="master-list">${(c.entries||[]).map((x,i)=>`<article class="master-list-item ${i===c.active?'active':''}"><b>${i===c.active?'▶ ':''}${esc(x.name)}</b><p>INI ${x.initiative} · PV ${x.hp||'—'} · ${esc(x.conditions||'sem condições')}</p><button data-del-combat="${i}">REMOVER</button></article>`).join('')||'<div class="gm-empty">A iniciativa ainda não começou.</div>'}</div></div>`;panel.querySelector('#combat-add').onsubmit=e=>{e.preventDefault();c.entries.push({name:panel.querySelector('#combat-name').value.trim(),initiative:Number(panel.querySelector('#combat-init').value||0),hp:Number(panel.querySelector('#combat-hp').value||0),conditions:panel.querySelector('#combat-cond').value.trim()});c.entries.sort((a,b)=>b.initiative-a.initiative);persistWorkbench();renderCombat(panel)};panel.querySelector('#combat-next').onclick=()=>{if(c.entries.length){c.active++;if(c.active>=c.entries.length){c.active=0;c.round=(Number(c.round)||1)+1}persistWorkbench();renderCombat(panel)}};panel.querySelector('#combat-prev').onclick=()=>{if(c.entries.length){c.active--;if(c.active<0){c.active=c.entries.length-1;c.round=Math.max(1,(Number(c.round)||1)-1)}persistWorkbench();renderCombat(panel)}};panel.querySelectorAll('[data-del-combat]').forEach(b=>b.onclick=()=>{c.entries.splice(Number(b.dataset.delCombat),1);c.active=Math.min(c.active,Math.max(0,c.entries.length-1));persistWorkbench();renderCombat(panel)});}
  function renderClues(panel){const w=workbench();panel.innerHTML=`<div class="master-work-grid"><form id="clue-form" class="master-card"><h4>Pista / revelação</h4><input id="clue-title" required placeholder="Título"><textarea id="clue-body" placeholder="Informação preparada"></textarea><button>GUARDAR PISTA</button></form><div class="master-list">${(w.clues||[]).map((x,i)=>`<article class="master-list-item"><b>${esc(x.title)}</b><p>${esc(x.body)}</p><small>${x.revealed?'REVELADA':'PRIVADA'}</small><div><button data-reveal="${i}">${x.revealed?'REVELAR NOVAMENTE':'REVELAR'}</button><button data-del-clue="${i}">REMOVER</button></div></article>`).join('')||'<div class="gm-empty">Nenhuma pista preparada.</div>'}</div></div>`;panel.querySelector('#clue-form').onsubmit=e=>{e.preventDefault();w.clues.push({id:crypto.randomUUID?crypto.randomUUID():'clue-'+Date.now(),title:panel.querySelector('#clue-title').value.trim(),body:panel.querySelector('#clue-body').value.trim(),revealed:false});persistWorkbench();renderClues(panel)};panel.querySelectorAll('[data-reveal]').forEach(b=>b.onclick=()=>{const x=w.clues[Number(b.dataset.reveal)];x.revealed=true;persistWorkbench();window.MS_SERVICES?.VTT?.event?.(tableId(),'reveal',{title:x.title,body:x.body});window.MS_PLATFORM?.toast('Pista revelada aos participantes.','success');renderClues(panel)});panel.querySelectorAll('[data-del-clue]').forEach(b=>b.onclick=()=>{w.clues.splice(Number(b.dataset.delClue),1);persistWorkbench();renderClues(panel)});}
  function renderWorld(panel){const w=workbench();panel.innerHTML=`<div class="master-work-grid"><form id="world-form" class="master-card"><h4>Consequência no mundo</h4><input id="world-faction" placeholder="Facção / local / ator"><input id="world-relation" placeholder="Relação ou estado"><textarea id="world-event" required placeholder="Acontecimento e consequência"></textarea><button>REGISTRAR CONSEQUÊNCIA</button></form><div class="master-list">${(w.world||[]).map((x,i)=>`<article class="master-list-item"><b>${esc(x.faction||'Mundo')}</b><small>${esc(x.relation||'')}</small><p>${esc(x.event)}</p><button data-del-world="${i}">REMOVER</button></article>`).join('')||'<div class="gm-empty">O mundo ainda não possui consequências registradas.</div>'}</div></div>`;panel.querySelector('#world-form').onsubmit=e=>{e.preventDefault();w.world.push({faction:panel.querySelector('#world-faction').value.trim(),relation:panel.querySelector('#world-relation').value.trim(),event:panel.querySelector('#world-event').value.trim(),at:new Date().toISOString()});persistWorkbench();renderWorld(panel)};panel.querySelectorAll('[data-del-world]').forEach(b=>b.onclick=()=>{w.world.splice(Number(b.dataset.delWorld),1);persistWorkbench();renderWorld(panel)});}
  ...
  function applyRemoteEvent(event){
    ...
    if(event.event_type==='reveal'){ window.MS_PLATFORM?.toast(`${p.title||'Pista revelada'}: ${p.body||''}`,'success'); return; }
