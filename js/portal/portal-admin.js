/* Mundos Sombrios — CMS local do Portal Oficial V0.61.3
   Somente ADM. Fonte única: mundosSombriosPortalContentV1 + PortalMedia.
*/
(function(){
  'use strict';
  ...
  async function renderForm(root,type,item=null){
    ...
    form.addEventListener('submit',async e=>{
      ...
      try{
        ...
        if(type==='featured')c.featured={...c.featured,title:String(fd.get('title')||'').trim(),subtitle:String(fd.get('subtitle')||'').trim(),description:String(fd.get('description')||'').trim(),category:String(fd.get('category')||'').trim(),world:String(fd.get('world')||'').trim(),status:'featured',media};
        else if(type==='hero')c.hero={...c.hero,eyebrow:String(fd.get('eyebrow')||'').trim(),title:String(fd.get('title')||'').trim(),subtitle:String(fd.get('subtitle')||'').trim(),description:String(fd.get('description')||'').trim(),primaryLabel:String(fd.get('primaryLabel')||'ENTRAR NO JOGO').trim(),media};
        else{
          const cfg=config[type];const payload={id:item?.id||`${type}-${Date.now().toString(36)}`,title:String(fd.get('title')||'').trim(),date:String(fd.get('date')||''),published:fd.get('published')==='on',[cfg.field]:String(fd.get('description')||'').trim(),media}; if(type==='story')payload.body=String(fd.get('body')||'').trim();
          const place=String(fd.get('world')||'').trim();
          if(type==='announcement')payload.category=place||'Atualização'; else if(type==='community')payload.kind=place||'Comunidade'; else if(type==='world')payload.key=place||item?.key||payload.id; else payload.world=place;
  ...
  function renderItems(root){
    ...
    root.querySelectorAll('[data-delete]').forEach(btn=>btn.addEventListener('click',async()=>{if(!confirm('Excluir este conteúdo?'))return;const [key,id]=btn.dataset.delete.split(':');const data=read();const item=(data[key]||[]).find(x=>String(x.id)===String(id));if(item?.media?.id){try{await PortalMedia.remove(item.media.id);}catch(_){}}data[key]=(data[key]||[]).filter(x=>String(x.id)!==String(id));write(data);renderItems(root);if(typeof window.renderOfficialPortal==='function')await window.renderOfficialPortal();}));
  }
