...
(function(){
  'use strict';
  const KEY='mundosSombriosPortalContentV1';
  const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const now=Date.now();
  const defaults={
    hero:{eyebrow:'PORTAL OFICIAL',title:'Mundos Sombrios',subtitle:'Dois mundos. Dois abismos. Uma história que não deveria ter sido aberta.',description:'Conheça o cenário, acompanhe as novidades, descubra classes e expansões e entre no sistema de criação de fichas quando estiver pronto.',primaryLabel:'ENTRAR NO JOGO'},
    featured:{title:'A Ordem dos Sete Arcanjos',subtitle:'Nova expansão em destaque',description:'Novos registros, caminhos e ameaças chegam ao acervo oficial.',category:'Expansão',world:'Ocultatun',status:'featured'},
    announcements:[
    ...
    worlds:[
      ...
      {id:'world-2',key:'ocultatun',title:'Ocultatun Ecos',eyebrow:'BIBLIOTECA DOS SELOS',description:'Um mundo de ritos, anomalias, símbolos e ecos que atravessam o conhecimento proibido.',accent:'arcane'}
    ],
  ...
  window.PortalContent={KEY,defaults,read,write,isAdmin,published,escapeHtml:esc,now};
})();
