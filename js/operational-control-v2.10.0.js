/* Mundos Sombrios — Fichas Globais + Central Operacional TRIPULAÇÃO V2.10.0 */
(function(){
  'use strict';
  ...
  async function resolveEvolutionRequest(kind,id,approved){
    ...
    try{if(kind==='evidence')await window.MS_EVOLUTION_BACKEND.resolveEvidence(id,approved,reason);else if(kind==='upgrade')await window.MS_EVOLUTION_BACKEND.resolveUpgrade(id,approved,null,reason);else if(kind==='development')await window.MS_EVOLUTION_BACKEND.resolveDevelopment(id,approved,null,reason);toast(approved?'Solicitação processada.':'Solicitação recusada.',approved?'success':'info');await loadCrew(true)}catch(error){toast(errorText(error),'error')}
  }
  ...
  async function refreshCounter(){if(!manager()||!context().table?.id)return;try{const rows=await svc()?.Games?.joinRequests?.(context().table.id);crewState.requests=unwrap(rows)||[];updateCrewCounter();}catch(_){}}
  function startPolling(){clearInterval(pollTimer);if(manager()&&context().table?.id){refreshCounter();pollTimer=setInterval(refreshCounter,15000);}}
  ...
  function bindRuntime(){
    ...
    window.MS_PLATFORM?.on?.('vtt:entered',()=>{startPolling();setTimeout(refreshCounter,100);});
    ...
    if(manager()&&context().table?.id)setTimeout(startPolling,80);
  }

  window.msOpenQuickSheets=openQuickSheets;
  window.msOpenCrewCenter=openCrewCenter;
  window.MS_OPERATIONAL_CONTROL=Object.freeze({version:'2.10.0',openQuickSheets,openCrewCenter,loadCrew,refreshCounter,createFabricLite,getState:()=>clone(crewState)});
